...
# -----------------------------
# Smart Loading Logic (No FileNotFoundError)
# -----------------------------
...
def load_all_resources():
    # Agar local laptop par .pkl file mil jati hai
    if os.path.exists(MODEL_PATH):
        with open(MODEL_PATH, "rb") as f:
            model = pickle.load(f)
        return model["vectorizer"], model["tfidf_matrix"], model["cosine_sim"], model["data"]
        ...
        data['combined_features'] = (
            ...
            data['City'].fillna('') + " " + 
            data['Locality'].fillna('')
        )
        vectorizer = TfidfVectorizer(stop_words='english')
        tfidf_matrix = vectorizer.fit_transform(data['combined_features'])
        cosine_sim = cosine_similarity(tfidf_matrix, tfidf_matrix)
        
        return vectorizer, tfidf_matrix, cosine_sim, data
...
st.markdown(
    ...
    <style>
        ...
        .css-1v0mbdj { background-color: #008b8b; color: white; }
        .css-1g1j4cx { background-color: #008b8b; color: white; }
        .stButton>button {
            background-color: #008b8b;
            color: white;
            border: none;
        ...
        h1 { color: #008b8b; }
        h3 { color: #555; }
        .css-ffhzg2 { font-weight: bold; color: #008b8b; }
    </style>
    """,
    unsafe_allow_html=True
)